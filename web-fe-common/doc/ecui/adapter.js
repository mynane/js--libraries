...
//{/if}//
    ...
    ecui = {
        ...
        dom: {
            /**
            * 兼容ie7,8的getElementsByClassName
            * @public
            ...
            */
            getElementsByClassName: function (el, className) {
                ...
                if (el.getElementsByClassName) {
                    results = el.getElementsByClassName(className);
                } else {
                    var childs = el.getElementsByTagName("*"), i, len, child;
                    for (i = 0, len = childs.length; i < len; i++) {
                        child = childs[i];
                        if (child.nodeType === 1 && this.hasClass(child, className)) {
                            results.push(child);
                        }
                    }
                }
            ...
            * @public
            * @param {HTMLElement} el Element 对象
            * @param {object} node 筛选对象 
            */
            getFather: function (el, node) {
                var results=[];
                var parentElem = el;
                while (parentElem) {
                    if (parentElem != null && parentElem != document.body) {
                            var isparent = true;
                            for (var i in node) {
                                if (!(i == "className" ? new RegExp("(^| )"+node[i]+"( |$)").test(parentElem[i]) : (i=="elem"?parentElem==node[i]:parentElem[i] == node[i]))) {
                                    isparent = false;
                                }
                            }
                            if (isparent) {
                ...
                if (ieVersion) {
                    ...
                    for (parent = dom.getParent(el); parent !== body; parent = dom.getParent(parent)) {
                        ...
                        if (!operaVersion) {
                            ...
                            top += util.toNumber(style.borderTopWidth) * 1 - parent.scrollTop;
                            left += util.toNumber(style.borderLeftWidth) * 1;
                            childStyle = style;
