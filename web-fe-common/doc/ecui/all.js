...
(function () {
    ...
    var ecuiName = 'ui',          // Element 中用于自动渲染的 ecui 属性名称
...
//                            event.target.focus();
    ...
    util.extend(core, {
        ...
        getOptions: function (el, attributeName) {
            ...
            if (text) {
                ...
                for (; /^(\s*;)?\s*(ext\-)?([\w\-]+)\s*(:\s*([^;\s]+(\s+[^;\s]+)*)?\s*)?($|;)/.test(text); ) {
                    ...
                    el = RegExp.$5;
                    (RegExp.$2 ? (options.ext = options.ext || {}) : options)[util.toCamelCase(RegExp.$3)] = RegExp.$4 ? el === 'true' ? true : el === 'false' ? false : el : true;
                }
         ...
         * @param {Function|Array} preprocess 控件正式生成前后对选项信息与主元素结构信息调整的预处理函数，如果是 Function 用于生成前的预处理，如果是 Array，分别表示前后的预处理
...
(function () {
     ...
     */
    function flush() {
        if (dialogs.splice(dialogs.indexOf(this), 1)[0]) {
            dialogs.push(this);
        }
